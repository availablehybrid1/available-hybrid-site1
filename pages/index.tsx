// pages/index.tsx — Home conectado a Google Sheets
...
type Vehicle = {
  ...
  description: string;
  photos: string[];
  status: "just_arrived" | "pending_detail" | "exclusive";
};
...
const formatPrice = (p: number | null) =>
  p !== null ? `$${p.toLocaleString()}` : "Consultar";

function getMeta(inv: Vehicle[]) {
  const makes = unique(inv.map((v) => v.make).filter(Boolean)).sort();
  const models = unique(
    inv
