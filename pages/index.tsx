// pages/index.tsx — Luxury / Legendary + Filtros + Badges
...
type Vehicle = {
  ...
  photos?: string[];
  // NUEVO (opcional):
  tags?: string[]; // ej.: ["Clean Title","1-Owner","New Tires","Fresh Oil","Smog Passed"]
  status?: "just_arrived" | "pending_detail"; // opcional
};

// -------- Helpers para filtros (3A) --------
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between px-3 py-2">
        <span
          className={`rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ring-1 ${statusClasses(
            v.status
          )}`}
        >
      ...
      <div className="p-4">
        ...
        {/* NUEVO: chips de características */}
        {tags.length > 0 && (
...
export default function Home() {
  ...
  // Inventario (acepta distintas formas de export en data/inventory.ts)
  ...
  // 3C) Meta + inicializar rangos de precio
