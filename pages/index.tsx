// Home — Luxury + filtros/orden + paginación (client-side)
import * as React from "react";
import { inventory as inv, type Vehicle } from "../data/inventory";
...
function buildMeta(data: Vehicle[]) {
  const makes = uniq((data.map(v => v.make).filter(Boolean) as string[])).sort();
  const mm = uniq(
    data.map(v => `${v.make ?? ""} ${v.model ?? ""}`.trim())
        .filter(Boolean)
  ).sort();
  const prices = data.map(v => v.price ?? 0).filter(n => n > 0);
  return {
    makes,
    models: mm,
...
export default function Home() {
  ...
  const meta = React.useMemo(() => buildMeta(inv), []);
  const [pmin, setPmin] = React.useState<number>(meta.minPrice);
  const [pmax, setPmax] = React.useState<number>(meta.maxPrice);
  ...
  const filtered = React.useMemo(() => {
    ...
    let arr = inv.filter(v => {
      ...
      const okMin = pmin ? price >= pmin : true;
      const okMax = pmax ? price <= pmax : true;
      return okMin && okMax;
    });
    ...
    return arr;
  }, [query, make, model, pmin, pmax, sortBy]);
  ...
  if (make)  chips.push({key:"make", label:make, clear:()=>{ setMake(""); setModel(""); }});
  if (model) chips.push({key:"model", label:model, clear:()=>setModel("")});
  if (pmin && pmin !== meta.minPrice) chips.push({key:"min", label:`Min $${pmin.toLocaleString()}`, clear:()=>setPmin(meta.minPrice)});
  if (pmax && pmax !== meta.maxPrice) chips.push({key:"max", label:`Max $${pmax.toLocaleString()}`, clear:()=>setPmax(meta.maxPrice)});

  const clearAll = () => {
    setQuery(""); setMake(""); setModel("");
    setPmin(meta.minPrice); setPmax(meta.maxPrice);
    setSortBy("price_desc"); setPerPage(12); setPage(1);
  };
            ...
            {(chips.length > 0) && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {chips.map(c => (
                  <button
                    key={c.key}
                    onClick={(e)=>{ e.preventDefault(); c.clear(); }}
